...
function Navbar({ showSearch, onSearch, showNotifications, onNotifications }) {
  const location = useLocation();
  const isCondensed =
    showSearch || showNotifications || location.pathname === "/messages";

  const linkClasses = ({ isActive }) =>
    `flex items-center space-x-3 transition ${isActive ? "font-bold" : ""}`;
  ...
  return (
    <div className="w-85 h-screen fixed top-0 left-0 border-r border-gray-700 p-6 flex flex-col space-y-6 bg-black text-white pt-10 justify-between">
      <div>
        <NavLink to="/">
          {!isCondensed ? (
            <svg
        ...
        <nav className="flex flex-col space-y-7 text-lg">
    ...
    ${showNotifications ? "font-bold text-pink-400" : ""}`}
      ...
      <nav className="flex flex-col space-y-7 text-lg">
        <NavLink to="/profile" className={linkClasses}>
          {({ isActive }) => (
            <>
              <HiMenu className="text-2xl" style={iconStyle(isActive)} />
        ...
        <NavLink to="/profile" className={linkClasses}>
          {({ isActive }) => (
            <>
              ...
              />
              {!isCondensed && (
                <span className="truncate">Autres applications Meta</span>
              )}
            </>
          )}
        </NavLink>
